...
win_position(1, [X,Y], [X1, Y], [X2, Y], [X3, Y], [X4, Y]) :- 
    X1 is X+1, 
    X2 is X+2, 
    X3 is X+3, 
    X4 is X+4.
...
win_position(2, [X,Y], [X, Y1], [X, Y2], [X, Y3], [X, Y4]) :- 
    Y1 is Y+1, 
    Y2 is Y+2, 
    Y3 is Y+3, 
    Y4 is Y+4.
...
win_position(3, [X,Y], [X1, Y1], [X2, Y2], [X3, Y3], [X4, Y4]) :- 
    X1 is X+1, Y1 is Y+1, 
    X2 is X+2, Y2 is Y+2, 
    X3 is X+3, Y3 is Y+3, 
    X4 is X+4, Y4 is Y+4.
...
win_position(4, [X,Y], [X1, Y1], [X2, Y2], [X3, Y3], [X4, Y4]) :-
    X1 is X+1, Y1 is Y-1, 
    X2 is X+2, Y2 is Y-2, 
    X3 is X+3, Y3 is Y-3, 
    X4 is X+4, Y4 is Y-4.
...
% ----+----+----+----+---- | ---+---+---+---+---
prav_dopln_5xA(P) :-
    s(S1, ' '), win_position(_, S1, S2, S3, S4, S5),
...
% ----+----+----+----+---- | ---+---+---+---+---
prav_dopln_5xB(P) :-
    s(S1, P), win_position(_, S1, S2, S3, S4, S5),
...
%     |    | S6 |    |     |    |   |   |   |   
prav_kriz(P):- 
    s(S1, ' '), win_position(ID1, S1, S2, S3, S4, S5),
    s(S2, P), s(S3, ' '), s(S4, P), s(S5, ' '),
    s(S6, ' '), win_position(ID2, S6, S7, S3, S8, S9), 
    ID1 \= ID2,
    s(S7, P), s(S8, P), s(S9, ' '),
    pohyb(S3, P, "Kriz").
